...
parfor iter = 1:runs
    disp(['the', num2str(iter),'RUN']);
    tic

    [Y,Y0,U,G] = generateData(BatchNum,N,4,0.1);
    ...
    g_Inherit_DC_temp = Inherit_DC(Y,U,sigma,n);
    g_Inherit_SS_temp = Inherit_SS(Y,U,sigma,n);
    for i = 1:N
    ...
    x0 = [1,0.80,0.00,0.00,0.5,0.00,0] ;
    LB = [0,0.72,-0.99,-0.99,0.00,0.00,0];
    UB = [1000,0.99,0.99,0.99,1,1,1];
    ...
    x0 = [1, 0.95,0.00,0.5,0.00,0];
    LB = [0, 0.90,-0.99,0.00,0.00,0];
    UB = [1000,1, 0.99,1,1,1];
